...
// Security FIELD
$security_field = isset($security_field) ? $security_field : '';
...
$label_security = setting('label-security');
$placeholder_security = strtoupper(substr(setting('site-name'),0, 1) . " ... " . substr(setting('site-name'), -1));
$label_message = setting('label-message');
$submit = setting('submit');
$reset = setting('reset');
$show_form = setting('show-form');
...
// IF INPUT SECURITY
if ( $security_field && strtolower(input()->post->security) != $security_field) {
  // Session Message
      session()->Message = ukAlertDanger(setting('label-security'));
      session()->redirect('./');
}
...
// Save Message to child page ... Template => ( contact-item )
    if ( setting('save-messages-page') ) {
            ...
            // https://processwire.com/api/ref/page/add-status/g/
                ...
                $p->save();
           // Save Log
                if(setting('save-messages-log')) {
                  wire('log')->save('e-mails', "$m_name | $m_from | $ip | $m_message",
                                    ['url' => "{$config->urls->admin}page/edit/?id=$p->id"]);
                }
      } else {
          // Save Log
                if(setting('save-messages-log')) {
...
// Session Message
    if ($session->Message) {
        ...
        echo "<a href='./' class='uk-button uk-button-primary'>$show_form</a>";
...
// CSRF Tokens
    ...
    $token_value = $this->session->CSRF->getTokenValue(); ?>
        ...
        <form name='cForm' id='contact-form' class='c-form' action='./' method='post' onsubmit="return validateForm()">
          <fieldset>
<!-- LEGEND -->
            <legend>
...
<!-- VERY SIMPLE SECURITY FIELD -->
            <label class='label-security uk-label uk-margin uk-text-danger uk-background-secondary' style='white-space: normal;'>
              * <?=$label_security?><br>
            <input class='uk-input uk-margin-small' name='security' placeholder='<?=$placeholder_security?>' type='text' required></label>
            <br>
<?php endif; ?>
