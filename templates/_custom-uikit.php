...
function ukPagination(PageArray $items, $options = array()) {
	...
	$next = isset($options['next']) ? $options['next'] : setting('next');
	$previous = isset($options['previous']) ? $options['previous'] : setting('previous');
...
function ukBlogPost(Page $page, $options = array()) {
	...
	$name = $page->createdUser->nickname; // Change name To Nickname ( Better Security )
	$body = $page->get('body');
	...
	if($options['summarize']) {
		...
		$title = "<a class='uk-button uk-button-text uk-text-large' href='$page->url'>$title</a>";
		$class = 'blog-post-summary';
		$heading = "<h3 class='uk-margin-remove'>$title</h3>";
		$summary = "<p class='uk-light'>$summary</p>";
		$body = explode('</p>', $body);
		$body = reset($body) . ' ';
		$body .= "<br><a class='uk-button uk-button-text' href='$page->url'>$options[moreText] $moreIcon</a></p>";
	} else {
		$class = 'blog-post-full';
		$heading = "<h1 class='uk-article-title uk-margin-remove'>$title</h1>";
		$summary = $summary ? $summary = "<h2 id='content-summary' class='uk-margin-small-top uk-h4'>$summary</h2>" : '';
	}
	...
	return "
<article class='uk-margin-small-bottom $class $bg_class' $style $dat_img>
		$overlay
		<div class='$overlay_class'>
			$heading
			<p class='uk-margin-small'>
				<span class='uk-article-meta uk-inline uk-light'>
					$metaIcon
					$byline
				</span>
				...
				<span class='num-comments uk-margin-small-left uk-text-muted uk-inline'>
					$numComments
				</span>
			</p>
			<div class='more-content uk-inline uk-card'>
				$summary
				$body
			</div>
		</div>
